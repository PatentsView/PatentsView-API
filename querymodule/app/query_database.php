...
{
    global $FIELD_SPECS, $groupVars;
    $selectString = '';
    foreach ($groupVars as $group) {
        ${$group['hasId']} = false;
        ${$group['hasFields']} = false;
    }
    ...
    foreach ($groupVars as $group) {
        if ($group['single'] == 'patent') {
            if (!${$group['hasId']})
                $selectFieldSpecs[$group['keyId']] = $FIELD_SPECS[$group['keyId']];
        }
        else {
            if (${$group['hasFields']} and !${$group['hasId']})
                $selectFieldSpecs[$group['keyId']] = $FIELD_SPECS[$group['keyId']];
        }
    }
